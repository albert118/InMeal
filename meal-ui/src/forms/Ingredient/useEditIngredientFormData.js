...
export default function useEditIngredientFormData() {
    ...
    function onUpdate(event) {
        const getTarget =
            valueUpdateStrategies[event.target.name] ??
            valueUpdateStrategies['default'];
    ...
    function onSubmit(event) {
        ...
        const shouldDelete =
            formData.isDeleted && existingIngredient.recipeUsageCount === 0;
    ...
    function getWarnings() {
        ...
        const recipePlural =
            existingIngredient.recipeUsageCount > 1 ? 'recipes' : 'recipe';
        const usagePlural =
            existingIngredient.recipeUsageCount > 1 ? 'usages' : 'usage';
