...
export default function useRecipeIngredients() {
    ...
    function executeNew(event, recipe) {
        ...
        if (id !== null) {
            console.warn(
                `adding a new recipe ingredient seems to have an existing Id '${id}'`
            );
        }
    ...
    function executeExisting(event, recipe) {
        ...
        return Promise.resolve({
            ...recipe,
            recipeIngredients: updateLocalIngredientsWithIds(
                newIngredients,
                recipe
            )
        });
    ...
    function executeUpdate(event, recipe) {
        ...
        // perform the update
        recipeIngredientsCopy.find(ri => ri['label'] === id).quantity =
            quantity;
...
function updateLocalIngredientsWithIds(newIngredients, recipe, apiData) {
    const recipeIngredients = newIngredients.map((newItem, idx) =>
        createRecipeIngredient(
            newItem.label,
            apiData ? apiData[idx].id : newItem.id,
            newItem?.units
        )
    );
